...
function applyRotation(x, y) {
    ...
    mainPhoto.style.transition = "transform 0.2s ease-out";
...
// ==========================================
// CONTROLE DA CÁPSULA DO TEMPO / LIVRO DIGITAL
// ==========================================

const capsuleModal = document.getElementById("capsuleModal");
const openCapsuleBtn = document.getElementById("openCapsule");
...
function updatePageVisibility() {
    ...
    if (pageIndicator) {
        pageIndicator.innerText = currentPage === 0 ? "Capa" : `Página ${currentPage} de ${pages.length - 1}`;
    }
}
...
if (openCapsuleBtn) {
    openCapsuleBtn.onclick = () => {
        capsuleModal.style.display = "flex";
    };
}

if (closeCapsuleBtn) {
    closeCapsuleBtn.onclick = () => {
...
function saveCapsuleData() {
    const inputs = document.querySelectorAll("#capsuleModal input, #capsuleModal textarea");
    inputs.forEach((input, i) => {
        if (input.type === "checkbox") {
            localStorage.setItem(`capsule_check_${i}`, input.checked);
        } else {
            localStorage.setItem(`capsule_text_${i}`, input.value);
        }
    });
    alert("Dados preservados na Cápsula!");
}
...
function loadCapsuleData() {
    ...
    inputs.forEach((input, i) => {
        if (input.type === "checkbox") {
            const saved = localStorage.getItem(`capsule_check_${i}`);
            if (saved !== null) input.checked = saved === "true";
        } else {
            const saved = localStorage.getItem(`capsule_text_${i}`);
            if (saved !== null) input.value = saved;
        }
    });
}

window.addEventListener("DOMContentLoaded", loadCapsuleData);
